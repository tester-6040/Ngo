...
function render_header(string $title): void
{
    ...
    <body class="bg-slate-100 text-slate-900 min-h-screen font-sans antialiased flex flex-col">
        ...
        <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3 text-sm">
                <?php if ($user): ?>
                    ...
                    <form method="post" action="<?= h(route_url('logout')) ?>" class="inline-block">
                        <input type="hidden" name="csrf_token" value="<?= h(csrf_token()) ?>">
                        <button class="bg-slate-900 hover:bg-slate-800 text-white px-4 py-2 rounded-lg">Logout</button>
                    </form>
                <?php else: ?>
...
function render_footer(): void
{
    ...
    <footer class="border-t border-slate-200/80 bg-white/80 mt-auto">
