...
## Base URL / Subfolder Deployments (Important)
- The app now auto-detects subfolder base path from `SCRIPT_NAME`.
- Example: if hosted at `http://localhost/Ngo-codex/`, route links become `/Ngo-codex/login`, `/Ngo-codex/dashboard`, etc.
- If your hosting has unusual rewrites, you can explicitly set `base_url` in `config/app.php`.
